...
export default class YModal extends React.Component {
  ...
  render() {
    ...
    const maskStyle = [
      style.mask,
      dark ? { backgroundColor: 'rgba(100, 100, 100, 0.7)' } : { backgroundColor: 'rgba(0, 0, 0, 0.5)' },
      {
    ...
    return (
      <Modal transparent={true} visible={this.state.modalVisible} onRequestClose={this.close} onShow={this.animateIn} {...props}>
        <Animated.Text
          style={maskStyle}
          onPress={() => {
  ...
  animateIn = () => {
    Animated.parallel([this.createAnimation(this.state.maskOpacity, 1), this.createAnimation(this.state.panelTranslateY, -this.state.panelHeight)]).start();
  };

  animateOut = () => {
    return new Promise((resolve, reject) => {
      Animated.parallel([this.createAnimation(this.state.maskOpacity, 0), this.createAnimation(this.state.panelTranslateY, 0)]).start(resolve);
    });
  };
}
